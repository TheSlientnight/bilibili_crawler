...
def get_av(url):
    ...
    for i in range(3):
        try:
            ...
            logger.debug("视频名称:{}, av号:{}".format(NAME, aid))
            return aid
...
# 获取弹幕
def get_barrage(viedo_url) -> str:
    """
    :param viedo_url:视频地址
    :return:爬取结果的xml文件
    """
    barrage_url = get_cid(viedo_url)
    HEADERS = {
    ...
    for i in range(3):
        try:
            logger.info(f"正在爬取{viedo_url}的弹幕")
            res = requests.get(barrage_url, headers=HEADERS).content.decode("utf-8")
            return res
        except Exception as e:
            logger.error(f"爬取{viedo_url}的弹幕失败，错误原因:{e}")
            continue
...
def get_comments(aid) -> list:
    ...
    for i in range(3):
        try:
            ...
            page_num = math.ceil(comment_count / 20)
            logger.debug(f"共计{page_num}页")
...
def save_comment(comment, file_name):
    ...
    try:
        logger.info("向{}_评论.csv中写入评论".format(file_name))
        pd_data.to_csv(f'../data/csv/{file_name}_评论.csv', index=False, header=False, mode='w', encoding="utf-8-sig")
    except Exception as e:
        logger.error(f"{file_name}.csv写入过程发生错误,错误原因: {e}")
    try:
        logger.info(f"对{file_name}_评论.csv内的评论进行处理")
        ...
        logger.error("处理评论时发生错误:{}".format(e))
...
def data_preprocess(barrage, file_name):
    ...
    try:
        logger.info(f"向{file_name}_弹幕.csv写入弹幕文件")
        pd_data.to_csv(f'../data/csv/{file_name}_弹幕.csv', index=False, header=False, mode='w', encoding="utf-8-sig")
    except Exception as e:
        logger.error(f"{file_name}_弹幕.csv写入过程发生错误,错误原因: {e}")
    try:
        logger.info(f"对{file_name}_弹幕.csv内的弹幕进行处理")
        ...
        logger.error("处理弹幕时发生错误:{}".format(e))
...
def count_words(text, file_name):
    ...
    pd_count.to_csv(f'../data/csv/{file_name}_词频.csv', mode='a', header=None)
